...
def test_shannon_entropy_calculation() -> None:
    ...
    single_lang = [
        LanguageStat(
            name="Rust", color="#dea584", bytes=10000, percentage=100.0
        )
    ]
    ...
    two_equal = [
        ...
        LanguageStat(
            name="Python", color="#3572A5", bytes=5000, percentage=50.0
        ),
    ]
    assert engine.compute_shannon_entropy(two_equal) == pytest.approx(
        1.0, abs=1e-3
    )
    ...
    four_equal = [
        ...
        LanguageStat(
            name="Python", color="#3572A5", bytes=2500, percentage=25.0
        ),
        LanguageStat(
            name="TypeScript", color="#3178c6", bytes=2500, percentage=25.0
        ),
        LanguageStat(
            name="Rust", color="#dea584", bytes=2500, percentage=25.0
        ),
    ]
    assert engine.compute_shannon_entropy(four_equal) == pytest.approx(
        2.0, abs=1e-3
    )
...
def test_polyglot_diversity_index() -> None:
    ...
    single = [
        LanguageStat(
            name="C++", color="#f34b7d", bytes=1000, percentage=100.0
        )
    ]
    ...
    two_equal = [
        ...
        LanguageStat(
            name="Python", color="#3572A5", bytes=500, percentage=50.0
        ),
    ]
    assert engine.compute_polyglot_diversity(two_equal) == pytest.approx(
        0.5, abs=1e-3
    )
...
def test_diurnal_metrics_fourier_analysis() -> None:
    ...
    # All commits on Sunday (2026-01-04 was a Sunday -> dow=0)
    sun_commits = [
        ContributionDay(date="2026-01-04", contribution_count=10),
        ContributionDay(date="2026-01-11", contribution_count=10),
    ]
    req_sun = UserPlanetProfileRequest(
        username="sun_dev", activity_heatmap=sun_commits
    )
    phase_sun, coherence_sun = engine.compute_diurnal_metrics(req_sun)
    assert phase_sun == pytest.approx(0.0, abs=1e-3)
    assert coherence_sun == pytest.approx(1.0, abs=1e-3)

    # All commits on Wednesday (2026-01-07 was a Wednesday -> dow=3 -> 3/7 ≈ 0.4286)
    wed_commits = [
        ContributionDay(date="2026-01-07", contribution_count=10),
        ContributionDay(date="2026-01-14", contribution_count=10),
    ]
    req_wed = UserPlanetProfileRequest(
        username="wed_dev", activity_heatmap=wed_commits
    )
    phase_wed, coherence_wed = engine.compute_diurnal_metrics(req_wed)
    assert phase_wed == pytest.approx(3.0 / 7.0, abs=1e-3)
    assert coherence_wed == pytest.approx(1.0, abs=1e-3)

    # Uniform daily commits across a full 7-day week
    uniform_week = [
        ContributionDay(date=f"2026-01-0{i}", contribution_count=5)
        for i in range(1, 8)
    ]
...
def test_repo_gini_index() -> None:
    ...
    assert (
        engine.compute_repo_gini_index(
            [
                LandformRepo(
                    name="solo", stars=100, forks=10, commit_count=50
                )
            ]
        )
        == 0.0
    ...
    assert engine.compute_repo_gini_index(equal_repos) == pytest.approx(
        0.0, abs=1e-3
    )
    ...
    skewed_repos = [
        LandformRepo(
            name="mega_star", stars=100000, forks=50000, commit_count=1000
        )
    ] + [
...
def test_full_profile_generation() -> None:
    ...
    req = UserPlanetProfileRequest(
        ...
        language_summary=[
            LanguageStat(
                name="Go", color="#00ADD8", bytes=8000, percentage=80.0
            ),
            LanguageStat(
                name="Python", color="#3572A5", bytes=2000, percentage=20.0
            ),
        ],
        landforms=[
